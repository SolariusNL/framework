...
export const verificationEmail = async (userId: number, email: string) => {
  ...
  sendMail(
    ...
    `
      ...
      token.id
    }" style="color: #0066cc; display: inline-block; text-decoration: none; background-color: #0066cc; color: #fff; padding: 12px 18px; border-radius: 12px; text-align: center;">Verify Email</a>
      </div>
