...
class UserRouter {
  ...
  @Post("/@me/checklists/create")
  @Authorized()
  public async createChecklist(
    @Account() user: User,
    @Body() data: ChecklistCreateBody
  ) {
  ...
  @Post("/@me/checklists/:id/tasks/create")
  @Authorized()
  public async createChecklistTask(
    ...
    if (
      ...
      description.length < 3 ||
      schedule
        ? new Date(schedule as Date).getTime() < Date.now()
        : false || tags
        ? !Array.isArray(tags)
        : false || tags
    ...
    const task = await prisma.checklistItem.create({
      data: {
        ...
        scheduled: schedule ? new Date(schedule as Date) : undefined,
        tags: tags
          ? {
              set: tags,
            }
  ...
  @Post("/@me/checklists/:id/tasks/:taskId/update")
  @Authorized()
  public async updateChecklistTask(
    ...
    const { schedule, finished } = data;
    if (
      schedule
        ? new Date(schedule as Date).getTime() < Date.now()
        : false || finished
        ? typeof finished !== "boolean"
        : false || !Object.keys(data).length
    ) {
      return {
        success: false,
  ...
  @Post("/@me/checklists/:id/tasks/:taskId/delete")
  @Authorized()
